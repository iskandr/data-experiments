...
def evaluate_lenet5(learning_rate=0.01, n_epochs=200,
                    nkerns=[20, 50], batch_size=500, validation_prct = 0.1):
    ...
    xtrain, ytrain, test_set_x, test_set_y  = load_data()
    ...
    n_test_batches = ntest / batch_size 
    n_train_batches = (ntrain-nvalid) / batch_size
    n_valid_batches = nvalid / batch_size
    ...
    ishape = (ncolors, image_rows, image_cols)
    ...
    layer0_input = x.reshape((batch_size, ncolors, image_rows, image_cols))
    ...
    layer0 = LeNetConvPoolLayer(rng, input=layer0_input,
            image_shape=(batch_size, ncolors, image_rows, image_cols),
            filter_shape=(nkerns[0], ncolors, 5, 5), poolsize=(2, 2))
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for i in xrange(n_train_batches):
            ...
            minibatch_x = train_set_x[i*batch_size:(i+1)*batch_size, :, :, :]
            minibatch_y = train_set_y[i*batch_size:(i+1)*batch_size]         
            cost_ij = train_model(minibatch_x, minibatch_y)
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                def run_model(model, xset, yset):
                  loss = 0
                  n = len(yset) / batch_size
                  for j in xrange(n):
                    xslice = xset[j*batch_size:(j+1)*batch_size, :, :, ]
                    yslice = yset[j*batch_size:(j+1)*batch_size]
                    loss += model(xslice, yslice)
                  return loss / n 

                this_validation_loss = run_model(validate_model, valid_set_x, valid_set_y)
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    test_score = run_model(test_model, test_set_x, test_set_y)
