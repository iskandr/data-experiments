...
def get_timings(X,Y, Xtest = None, Ytest = None): 
  ...
  results = {} 
  Result = namedtuple('Result', ('train_time', 'test_time', 'total_time', 'train_accuracy', 'test_accuracy'))
  for name, model in sorted(learning_algorithms.items()):
    ...
    try:
      ...
      test_accuracy = accuracy(model.predict(Xtest), Ytest)
      t2 = timeit.default_timer()
      train_accuracy = accuracy(model.predict(Xtrain), Ytrain)
      train_time = t1 - t0
      test_time = t2 - t1
      result = Result(train_time, test_time, train_time + test_time, train_accuracy, test_accuracy)
